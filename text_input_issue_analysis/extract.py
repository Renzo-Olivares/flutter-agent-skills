...
repo_dir = "flutter-agent-skills/text_input_issue_analysis"
...
with open(os.path.join(repo_dir, "batch_v4_2.json"), "w") as f:
    json.dump(batch_out, f, indent=2)
    
for i in batch:
    print(f"#{i['number']}: {i['title']} (R={i.get('reactions', {}).get('total', 0)})")
